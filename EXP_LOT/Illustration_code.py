...
def Illustrative_example(n,m,sigma_x1=1,sigma_x2=1,sigma_y1=2,sigma_y2=2,seed=49):
    xa = np.arange(n)
    a = np.exp(-(xa-int(n/3))**2/sigma_x1**2) + 1.5 * np.exp(-(xa-int(5*n/6))**2/sigma_x2**2)
    a = a / np.sum(a)

    xb = np.arange(m)
    b = 2 * np.exp(-(xb-int(m/5))**2/sigma_y1**2) +  np.exp(-(xb-int(2*m/4))**2/sigma_y2**2)
    b = b /np.sum(b)
...
v_min = 1
for reg in regs:
    res, acc_Sin, times_Sin, P_Sin = LinSinkhorn.Sinkhorn(C,reg,a,b,max_iter=1000,delta=1e-9,lam=0,time_out=200)
...
for rank in ranks:
    res, acc_LOT, times_LOT, num_ops, Q, R, g = LinSinkhorn.Quad_LOT_MD(X,Y,a,b,rank,0,alpha,C,C_init=C_init,Init=Init,seed_init=seed_init,reg_init=reg_init,gamma_init=gamma_init,gamma_0=gamma_0,method=method,max_iter=max_iter,delta=delta,max_iter_IBP=10000,delta_IBP=1e-9,lam_IBP=0,time_out=100)
...
heights = [1, 3, 3] #nrows
gs = fig.add_gridspec(nrows=3, ncols=5, width_ratios=widths,
...
for k in range(len(list_P_Sin)):
    ...
    if k == 0:
        ...
        f_ax4.imshow(list_P_Sin[- 1 - k], interpolation='nearest',cmap='Greys',aspect='auto')

        f_ax5 = fig.add_subplot(gs[2, k+1],sharey=f_ax2)
        f_ax5.imshow(list_P_LOT[k], interpolation='nearest',cmap='Greys',aspect='auto')
    else:

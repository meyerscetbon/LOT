...
# load data
adata = read_dataset(VAR_DS_PATH, obs=[CELL_DAYS_PATH, CELL_GROWTH_PATH], obs_filter=SERUM_CELL_IDS_PATH)
adata.shape
...
for ind_day, day in enumerate(unique_days[:-1]):
    ...
    tmp_3 = adata[adata.obs['day'] == unique_days[ind_day]].obs['cell_growth_rate'].values
    X = PCA(n_components=n_components).fit_transform(np.concatenate([tmp_1,tmp_2],axis=0))
...
CELL_DISTRIBUTION_IPSC_DAY18 = CELL_DISTRIBUTION_IPSC_DAY18 / np.sum(CELL_DISTRIBUTION_IPSC_DAY18)


### Compute the Optimal transport ###
Square_Euclidean_cost = lambda X,Y: LinSinkhorn.Square_Euclidean_Distance(X,Y)
Square_Euclidean_factorized_cost = lambda X,Y: LinSinkhorn.factorized_square_Euclidean(X,Y)
...
for i, day in enumerate(unique_days[start:]):
  if day == 18.:
    continue
  print('\r' + f'Executing Sinkhorn and LOT between the days {day} and {unique_days[start + i+1]} / 18', end='')
  ...
  res_Sin, acc_Sin, times_Sin, P_Sin = LinSinkhorn.Sinkhorn_LSE(C,epsilon,a,b,max_iter=max_iter_Sin,delta=delta_Sin,lam=0,time_out=100)
  dict_results_Sin[day] = P_Sin


  ### LOT methods ###
  ### LOT with PCA ###
  res_LR, acc_LR_PCA, times_LR, Q, R, g = LinSinkhorn.Lin_LOT_MD(PCA30_SERUM_DAY[day][0],PCA30_SERUM_DAY[day][1],a,b,rank_LR,reg,alpha,cost,cost_factorized,Init_trivial=Init_trivial,Init_random=Init_random,seed_init=seed_init,reg_init=reg_init,gamma_init=gamma_init,gamma_0=gamma_0,method=method,max_iter=max_iter_LR,delta=delta_LR,max_iter_IBP=10000,delta_IBP=1e-3,lam_IBP=0,time_out=100)
...
for i, day in enumerate(reverse_days):
  ...
  cell_distribution_ipsc_Sin[reverse_days[i+1]] = cell_distribution_Sin / np.sum(cell_distribution_Sin)
  ...
  cell_distribution_ipsc_LOT_PCA[reverse_days[i+1]] = cell_distribution_LOT_PCA / np.sum(cell_distribution_LOT_PCA)
...
#                        norm=mpl.colors.Normalize(vmin=0, vmax=18))
# plt.show()
list = [9.5,5,9,15.5,6,8,16,6.5,12,12,14,15.5,9.5,6,7,9,8.5,6,10,18,8.5]
